...
export const metadata: Metadata = { title: 'Teach-back report' }

interface TurnEntry { role: 'student' | 'teacher'; text: string }

export default async function FeynmanReportPage(
  ...
  const session = await prisma.feynmanSession.findUnique({ where: { id } })
  if (!session || session.userId !== user.id) notFound()

  const report = session.report as unknown as FeynmanReport | null
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <main>
        {report && session.status === 'COMPLETED' ? (
          <ReportScreen report={report} concept={session.concept} />
        ) : (
          <div className="mx-auto flex max-w-md flex-col items-center gap-4 px-4 py-16 text-center">
            <p className="text-lg font-semibold">
              {session.status === 'FAILED' ? 'This teach-back was not scored' : 'This teach-back has no report yet'}
            </p>
            <p className="text-sm text-muted-foreground">Your explanation is saved.</p>
          </div>
        )}
          ...
          <div className="mx-auto w-full max-w-3xl px-4 pb-12">
            ...
            <div className="space-y-3 rounded-2xl border-2 border-border bg-card p-4 shadow-sm">
              {transcript.map((t, i) => {
                ...
                return (
                  <div key={i} className={cn('flex flex-col', isTeacher ? 'items-end' : 'items-start')}>
