...
import {
  ...
  unlockAudio,
  playGoSting,
} from "@/lib/visualizer/sound";
import { authClient } from "@/lib/auth-client";
import {
  AiTutor,
  type TutorLang,
  type TutorRequest,
} from "@/components/visualizer/ai-tutor";
...
} from "@/components/visualizer/code-editor";
...
import {
  ...
  type SubmitResult,
} from "@/components/visualizer/challenge-setup";
...
import {
  PracticeCheck,
  type CheckResult,
} from "@/components/visualizer/practice-check";
...
function safeInterpret(source: string): {
  trace: Trace | null;
  error: string | null;
...
export default function Home() {
  ...
  const changeAiLang = useCallback((l: TutorLang) => {
    setAiLang(l);
    try {
      window.localStorage.setItem("phi-viz-ai-lang", l);
    } catch {
      /* ignore */
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      setGuestDismissed(
        window.localStorage.getItem("phi-viz-guest-dismissed") === "1",
      );
    } catch {
      /* ignore */
    }
  ...
  const dismissGuestBanner = useCallback(() => {
    ...
    try {
      window.localStorage.setItem("phi-viz-guest-dismissed", "1");
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const [openTopics, setOpenTopics] = useState<Set<TopicId>>(
    new Set([PROBLEM_TOPICS[0].id]),
  );
  ...
  const [bugCompletedIds, setBugCompletedIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const loadProgress = useCallback(async () => {
    try {
      ...
      /* progress is a nicety — never block the lab on it */
    }
  }, []);
  ...
  const { settings, setFeature, resetAll, enabledCount } =
  ...
  useEffect(() => {
    try {
      ...
      realEngineRef.current = settings.realEngine;
    }
  }, [settings.realEngine]);
  ...
  const runQuiet = useCallback((source: string) => {
    ...
    // still surface.
    const suppress =
      err != null && realEngineRef.current && getParseError(source) == null;
  ...
  const applyRun = useCallback(
    (t: Trace | null, err: string | null, line: number | null) => {
      setTrace(t);
      setError(err);
      setErrorLine(line);
      setCurrentIndex(0);
  ...
  const runAndPlay = useCallback(
    (source: string) => {
      syncCodeToLocation(source);
      if (realEngineRef.current) {
        setRealEngineBusy(true);
        fetchQjsTrace(source)
          .then(({ trace: t, error: err, errorLine: line }) =>
            applyRun(t, err, line),
          )
          .catch((e) =>
            applyRun(null, e instanceof Error ? e.message : String(e), null),
          )
  ...
  useEffect(() => {
    ...
    const example = wanted
      ? DEMO_EXAMPLES.find((ex) => ex.id === wanted)
      : undefined;
    if (example) {
  ...
  useEffect(() => {
    ...
    const dwell =
      speed <= 1 && (kind === "enter" || kind === "condition") ? 1.7 : 1;
    const calm = settings.calmMode ? 1.6 : 1;
    timerRef.current = setTimeout(
      () => advanceRef.current(),
      (700 * dwell * calm) / speed,
    );
    return () => {
  ...
  useEffect(() => {
    if (!signedIn || !trace || lastIndex <= 0 || currentIndex !== lastIndex)
      return;
    ...
    if (
      problem?.kind === "demo" &&
      !problemAwardedRef.current.has(problem.id)
    ) {
      problemAwardedRef.current.add(problem.id);
      void awardXp("viz_problem", `problem:${problem.id}`, {
  ...
  const resolveQuiz = useCallback(
    (correct: boolean) => {
      ...
      setStreak((s) => {
        ...
        if (correct) {
          void awardXp("quiz_correct", `${codeHash(code)}:${q.stepIndex}`, {
            streak: ns,
          });
        }
        return ns;
  ...
  const buildErrorRequest = useCallback(
    (): TutorRequest => ({ mode: "error", code, error: error ?? "" }),
    [code, error],
  );
  ...
  const changedVars = useMemo(() => {
    ...
    const sig = (frameName: string, name: string, v: string) =>
      `${frameName}:${name}=${v}`;
    const prev = new Map<string, string>();
    for (const f of previousStep.frames) {
      for (const v of f.vars)
        prev.set(`${f.name}:${v.name}`, JSON.stringify(v.value));
    }
  ...
  const indexPointers = useMemo(() => {
    ...
    for (const f of currentStep.frames) {
      for (const v of f.vars) {
        if (
          v.value.t === "prim" &&
          typeof v.value.v === "number" &&
          Number.isInteger(v.value.v)
        ) {
    ...
    for (const f of currentStep.frames) {
      for (const v of f.vars) {
        ...
        for (const idxName of idxNames) {
          ...
          if (val === undefined || val < 0 || val >= (h.cells?.length ?? 0))
            continue;
  ...
  const panelTabs = useMemo(() => {
    ...
    for (const f of TAB_FEATURES) {
      if (settings[f.key])
        tabs.push({ id: f.view, key: f.key, label: f.label, icon: f.icon });
    }
    return tabs;
  }, [settings]);
  ...
  const addableTabs = useMemo(
    () => TAB_FEATURES.filter((f) => !settings[f.key]),
    [settings],
  );
  ...
  const effectiveView: PanelView = panelTabs.some((t) => t.id === view)
    ? view
    : "memory";
  ...
  const editorHighlight = useMemo<EditorHighlight | null>(() => {
    ...
    const signature = currentStep.bindings
  ...
  const handleUseChallenge = useCallback(
    (newCode: string) => {
      setActiveExampleId("");
      setCode(newCode);
      runAndPlay(newCode);
  ...
  const [challengeResult, setChallengeResult] = useState<SubmitResult | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (t) {
        ...
        if (Array.isArray(arr)) setChallengeTopics(arr.filter(isTopic));
      }
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        "phi-viz-ch-topics",
        JSON.stringify(challengeTopics),
      );
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    /* eslint-disable react-hooks/set-state-in-effect */
    try {
      if (window.localStorage.getItem("phi-viz-ai-charge-ack") === "1")
        setAiChargeAck(true);
    } catch {
      /* ignore */
    }
  ...
  const requestAiCharge = useCallback(async (): Promise<boolean> => {
    ...
    if (userXp < 30) {
      toast.error("You need 30 XP for AI help");
      return false;
    }
    if (aiChargeAck) {
      setTimeout(() => void refreshXp(), 2500);
      return true;
    }
    return new Promise<boolean>((resolve) => {
      aiResolveRef.current = (ok: boolean) => {
        if (ok) setTimeout(() => void refreshXp(), 2500);
        resolve(ok);
      };
  ...
  const buyHint = useCallback(async () => {
    setHintBusy(true);
    try {
      const res = await fetch("/api/labs/js-motion/challenge/hint", {
        method: "POST",
      });
      const data = await res.json();
      if (!res.ok) {
        toast.error(data?.message || "Could not get a hint");
        return;
      }
    ...
    fetch("/api/labs/js-motion/challenge/active")
      ...
      .then((d) => {
        if (alive && d?.active) {
          ...
          if (d.rescuable === "time" || d.rescuable === "life")
            setRescue({ kind: d.rescuable });
        }
      })
      .catch(() => {});
    return () => {
  ...
  const openChallenge = useCallback(() => {
    ...
    setChallengeSource(
      code.replace(/\/\/.*$/gm, "").trim() ? "code" : "topics",
    );
    setChallengePhase("setup");
  }, [code]);
  const closeChallenge = useCallback(() => {
    setChallengePhase("off");
    setChallenge(null);
    setChallengeResult(null);
    setRescue(null);
  }, []);
  ...
  const activateChallenge = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data?.message || "Could not start the challenge");
        return;
      }
      ...
      setHintUsed(false);
      setActiveExampleId("");
      const header = ch.signature
        ? `function ${ch.signature} {`
        : `function ${ch.fnName}() {`;
      ...
      fetch("/api/labs/js-motion/leaderboard")
        .then((r) => (r.ok ? r.json() : null))
        .then((d) => {
          prevRankRef.current = d?.you?.rank ?? null;
        })
        .catch(() => {});
    } catch {
  ...
  const submitChallenge = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === "rescue" && data.rescuable) {
        if (data.rescuable === "life")
          setChallenge((c) => (c ? { ...c, attemptsUsed: c.maxAttempts } : c));
      ...
      if (data.status === "active") {
        setChallenge((c) =>
          c ? { ...c, attemptsUsed: c.attemptsUsed + 1 } : c,
        );
  ...
  const visualPanel = (
    <section className="relative rounded-xl border-2 border-border bg-card overflow-hidden shadow-sm flex flex-col min-h-0 h-full">
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        ...
        {effectiveView === "flow" && (
          <FlowChart trace={trace} currentIndex={currentIndex} onJump={seek} />
        )}
        {effectiveView === "eventloop" && (
          <EventLoopPanel async={currentStep?.async} />
        )}
        {effectiveView === "closures" && <ClosureCapture step={currentStep} />}
        {effectiveView === "hoisting" && (
          <HoistingPanel hoisting={trace?.hoisting} step={currentStep} />
        )}
      </div>
      {settings.diffFlash && (
        <DiffFlash
      ...
      {challengeEntering && (
        <>
          <ArenaEntry
            ...
            onDone={() => setChallengeEntering(false)}
          />
          {challenge && (
            <StakeBurn stake={challenge.stake} calm={settings.calmMode} />
          )}
        </>
      )}
      ...
      <AiChargeDialog
        ...
        onConfirm={(dontShow) => {
          ...
          if (dontShow) {
            setAiChargeAck(true);
            try {
              window.localStorage.setItem("phi-viz-ai-charge-ack", "1");
            } catch {
              /* ignore */
            }
          }
        ...
        <div className="px-4 py-2.5 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button
              ...
              title="Open a blank editor for your own code"
            >
        ...
        <div className="shrink-0 border-b border-violet-500/20 bg-linear-to-r from-violet-500/10 via-fuchsia-500/10 to-transparent">
          <div className="px-4 py-2 flex items-center gap-3">
            ...
            <p className="text-xs text-foreground/80 leading-snug">
              You&apos;re exploring as a guest — everything here is open.{" "}
              <strong className="text-foreground">Sign in free</strong> to also
              unlock the AI tutor, quizzes and XP. No pressure!
            </p>
            <Link
              href="/sign-in?next=/labs/js-motion"
              ...
              <div className="flex-1 min-h-0">
                ...
                >
                  ...
                  >
                    <section
                      className={cn(
                        "h-full rounded-xl border-2 bg-card overflow-hidden shadow-sm flex flex-col min-h-0 relative",
                        challengeActive
                          ? "border-rose-500/60"
                          : "border-border",
                      )}
                    >
                      ...
                      {challengeActive && !settings.calmMode && (
                        ...
                        >
                          <div
                            ...
                            style={{
                              background:
                                "conic-gradient(from 0deg, transparent 0deg, rgba(244,63,94,0.95) 35deg, transparent 80deg, transparent 190deg, rgba(251,146,60,0.8) 225deg, transparent 270deg)",
                            }}
                          />
                        </div>
                      )}
                      <div className="relative z-30 flex items-center gap-2 px-3 py-2 border-b bg-muted/50 shrink-0">
                        ...
                        <span className="ml-auto text-[11px] text-muted-foreground">
                          {isPlaying
                            ? "read-only · playing"
                            : "⌘/Ctrl+Enter to run · click gutter for breakpoints"}
                        </span>
                      </div>
                      {settings.aiTutor &&
                        !sessionPending &&
                      ...
                      {!challengeActive &&
                        (activeBug || activeProblem?.kind === "practice") && (
                          <div className="shrink-0 px-2 pb-2 pt-1">
                            <PracticeCheck
                              title={
                                activeBug
                        ...
                        {error && (
                          <div className="pointer-events-auto flex items-start gap-2 p-2.5 rounded-lg border-2 border-rose-300 bg-rose-50/95 dark:bg-rose-950/90 backdrop-blur-sm text-rose-900 dark:text-rose-200 text-sm shadow-lg max-h-40 overflow-y-auto">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="font-mono text-xs mt-1 wrap-break-word">
                                {error}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="pointer-events-auto flex items-start gap-2 p-2.5 rounded-lg border-2 border-amber-300 bg-amber-50/95 dark:bg-amber-950/90 backdrop-blur-sm text-amber-900 dark:text-amber-200 text-sm shadow-lg max-h-40 overflow-y-auto">
                            ...
                            <div className="min-w-0">
                              <div className="font-semibold">
                                Stopped early — possible infinite loop
                              </div>
                              <div className="text-xs mt-1 wrap-break-word">
                                {trace.stopReason} Showing the first{" "}
                                {trace.steps.length - 1} steps so you can see
                                what happened.
                              </div>
                            </div>
                          </div>
