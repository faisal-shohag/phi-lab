  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-zinc-50 via-white to-zinc-100 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950">
      <header className="sticky top-0 z-10 border-b bg-background/70 backdrop-blur-sm">
        <div className="mx-auto flex max-w-5xl items-center gap-3 px-4 py-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] leading-tight text-muted-foreground">Spoken English for developers</p>
          </div>
      ...
      <main className="relative overflow-hidden">
        <div className="pointer-events-none absolute -top-24 -left-24 h-96 w-96 rounded-full bg-sky-400/20 blur-3xl dark:bg-sky-500/10" aria-hidden />
        <div className="pointer-events-none absolute top-1/3 -right-32 h-96 w-96 rounded-full bg-teal-500/20 blur-3xl dark:bg-teal-500/10" aria-hidden />
        <div className="pointer-events-none absolute bottom-0 left-1/4 h-72 w-72 rounded-full bg-cyan-400/10 blur-3xl dark:bg-cyan-500/10" aria-hidden />
        ...
        <AnimatePresence mode="wait">
          ...
          {en.phase === 'greenroom' && (
            <motion.div key="greenroom" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <GreenRoom scenario={en.scenario} connecting={false} onStart={en.start} onBack={en.reset} />
            </motion.div>
          )}

          {(en.phase === 'connecting' || en.phase === 'live' || en.phase === 'reconnecting') && (
          ...
          {en.phase === 'generating' && (
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Checking fluency, clarity and how to say things better.</p>
              </div>
            </motion.div>
          )}
          ...
          {en.phase === 'report' && en.report && (
            <motion.div key="report" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <ReportScreen
                ...
                scenario={en.scenario}
                onNew={en.reset}
                onRetry={() => { if (en.scenario) en.enterGreenRoom(en.scenario) }}
              />
            </motion.div>
          )}
          ...
          {en.phase === 'error' && (
            ...
            >
              ...
              <div className="max-w-md">
                <p className="text-lg font-semibold">{resolveErrorCopy(en.errorCode).title}</p>
                <p className="mt-1 text-sm text-muted-foreground">{en.error ?? resolveErrorCopy(en.errorCode).message}</p>
              </div>
              <div className="flex items-center gap-2">
                {en.errorCode === 'REPORT_FAILED' && en.canRetryReport && (
                  <Button onClick={en.retryReport}>Try scoring again</Button>
                )}
                <Button variant={en.errorCode === 'REPORT_FAILED' ? 'outline' : 'default'} onClick={en.reset}>
                  Back to start
                </Button>
              </div>
            </motion.div>
