    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <ul className="space-y-1">
            ...
            <li>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="truncate text-sm font-semibold">Challenge mode</span>
        ...
        <button
          ...
          disabled={!nextBug}
          className={cn(
            'mt-2 flex w-full items-center gap-3 rounded-xl border-2 border-dashed border-border p-2.5 text-left',
        ...
        {nextProblem && (
          <button
            ...
            className="mt-3 w-full rounded-xl bg-linear-to-r from-pink-500 to-red-500 px-3 py-2.5 text-left text-white shadow-lg shadow-pink-500/20 hover:opacity-90"
          >
            <span className="block text-[10px] font-bold uppercase tracking-wide text-white/70">Next up</span>
            <span className="block truncate text-sm font-bold">{nextProblem.title}</span>
            <span className="block truncate text-[11px] text-white/80">{nextProblem.description}</span>
          </button>
        )}
        {!nextProblem && (
          <p className="mt-3 rounded-xl bg-emerald-500/10 py-3 text-center text-sm font-semibold text-emerald-600 dark:text-emerald-400">
